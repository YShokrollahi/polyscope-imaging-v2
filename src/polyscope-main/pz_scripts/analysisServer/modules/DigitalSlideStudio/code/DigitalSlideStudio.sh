...
# 2.1 added a filesize check to avoid 0 sized files
# 2.2 fixed filesize check (-s instead of -n)
# 2.3 Fixed Ss1.jpg - now totallayeramount -3 
...
BFTOOLS=/home/molecpath/aheindl/scratch/polyzoomer/bftools
GREP_CMD=/bin/grep
...
RETRYEXTRACT=3 #how often should tiles tried to be extracted? (0 file size problem)
...
tIMGWIDTH=`${BFTOOLS}/showinf -nopix "${1}" | $GREP_CMD "Width = "`
IMGWIDTH=`$GREP_CMD -m 1 -o "[0-9]*" <<< "$tIMGWIDTH"`  #stop after the first match
tIMGHEIGHT=`${BFTOOLS}/showinf -nopix "${1}" | $GREP_CMD "Height = "` # - " -
IMGHEIGHT=`$GREP_CMD -m 1 -o "[0-9]*" <<< "$tIMGHEIGHT"`

#Get width and height
tIMGWIDTHORIG=`${BFTOOLS}/showinf -nopix "${1}" | $GREP_CMD "Width = "`
IMGWIDTHORIG=`$GREP_CMD -m 1 -o "[0-9]*" <<< "$tIMGWIDTHORIG"`  #stop after the first match
tIMGHEIGHTORIG=`${BFTOOLS}/showinf -nopix "${1}" | $GREP_CMD "Height = "` # - " -
IMGHEIGHTORIG=`$GREP_CMD -m 1 -o "[0-9]*" <<< "$tIMGHEIGHTORIG"`

#Get total amount of series
tTOTALAMOUNTOFSERIES=`${BFTOOLS}/showinf -nopix "${1}"  | $GREP_CMD "Series #" | tail -1`
TOTALAMOUNTOFSERIES=`$GREP_CMD -m 1 -o "[0-9]*" <<< "$tTOTALAMOUNTOFSERIES"`
 ...
 then
  ...
  for (( w = 0 ; w < ${IMGWIDTH} ; w= w + ${TILESIZE} ))
	...
	TILESIZEY=`expr $IMGHEIGHT - $h` #save remaining pixel in smaller tile
      ...
      if [[ -s "${WORKINGDIR}/Da${TILECOUNTER}.${EXPORTFILEFORMAT}" ]]
      then
         echo "File size OK!"
       else
         echo "File size=0! retry"
         for retExt in {1..$RETRYEXTRACT}
         do
           if [[ -s "${WORKINGDIR}/Da${TILECOUNTER}.${EXPORTFILEFORMAT}" ]]
           then
             break #end retry cycles in case tile has been extracted correctly             
           else
             echo "Retry tile extracting"
             ${BFTOOLS}/bfconvert -overwrite -series 0 -crop ${w},${h},$TILESIZEX,$TILESIZEY "$1" "${WORKINGDIR}/Da${TILECOUNTER}.${EXPORTFILEFORMAT}"             
           fi       
         done           
      fi            
...
sed -i.bak "s/%TILESIZE%/${TILESIZE}/g" "${WORKINGDIR}/FinalScan.ini"
sed -i.bak "s/%XSIZE%/${IMGWIDTH}/g" "${WORKINGDIR}/FinalScan.ini"
sed -i.bak "s/%YSIZE%/${IMGHEIGHT}/g" "${WORKINGDIR}/FinalScan.ini"

#Create overview image (total amount of series - 1)  # changed to -3 
PREVIEWWIDTH=`${BFTOOLS}/showinf -nopix "${1}" |  $GREP_CMD "Width = " | tail -3 | $GREP_CMD -m 1 "Width = " | $GREP_CMD -o "[0-9]*" -`
PREVIEWHEIGHT=`${BFTOOLS}/showinf -nopix "${1}" | $GREP_CMD "Height = " | tail -3 | $GREP_CMD -m 1 "Height = " | $GREP_CMD -o "[0-9]*" -`
PREVIEWSERIE=1
...
echo "montage -limit area 12192 -limit memory 8192 \\" > "${WORKINGDIR}/_daTile.sh" 
...
LABELWIDTH=`${BFTOOLS}/showinf -nopix "${1}" | $GREP_CMD "Width = " | tail -2 | head -n 1 | $GREP_CMD -o "[0-9]*" -`
LABELHEIGHT=`${BFTOOLS}/showinf -nopix "${1}" | $GREP_CMD  "Height = " | tail -2 | head -n 1 | $GREP_CMD -o "[0-9]*" -`
GETLABELSERIES=`${BFTOOLS}/showinf -nopix "${1}" | $GREP_CMD "Series" | tail -2 | head -n 1 | $GREP_CMD -o "[0-9]*" -`
