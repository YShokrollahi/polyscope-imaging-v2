...
/*
	...
	Last Date: 2015.08.20 15:38:28 (+02:00)
	Version: 0.0.9
*/
...
require_once 'getLsfJobList.php';
...
/*	private $jobStati = array(
	...
	public function update() {
		...
		foreach($jobs as $guid) {
			...
			if(isJobInQueue($guid, $currentJobQueue)) {
				jobLog($guid, "[SKIPPING] Job is already in Queue!");
				continue;
			}
	...
	public function handleToMergeJobs(&$jobs) {
		...
		if($freeSlots > 0) {
			...
			foreach($entriesToProcess as $index) {
				...
				$command = 'php startJob.php doMerge.php ' . enclose( base64_encode(json_encode( $guid )) );
	...
	public function handleToAnalyzeJobs(&$jobs) {
		...
		if($freeSlots > 0) {
			...
			foreach($entriesToProcess as $index) {
				...
				$command = 'php startJob.php doAnalysis.php ' . enclose( base64_encode(json_encode( $guid )) );
	...
	public function handleUploadedJobs(&$jobs) {
		...
		if($freeSlots > 0) {
			...
			foreach($entriesToProcess as $index) {
				...
				$command = 'php startJob.php doTiling.php ' . enclose( base64_encode(json_encode( $guid )) );
