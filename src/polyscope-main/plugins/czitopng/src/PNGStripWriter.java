...
public class PNGStripWriter extends ImageWriter {
    ...
    public PNGStripWriter(ImageWriterSpi originatingProvider) {
...
"The chunk will not be written.");
    ...
    private static int chunkType(String typeString) {
     ...
     private void encodePass(ImageOutputStream os,
        ...
        RenderedImage image = ImageIO.read(files[0]);
        ...
        if (metadata.IHDR_colorType == PNG_COLOR_GRAY_ALPHA &&
        ...
        for (int imageNumber=0; imageNumber<files.length; imageNumber++) {
          image = ImageIO.read(files[imageNumber]);
          int imHeight = image.getHeight();
          
          for (int row = trueY + yOffset; row < minY + imHeight; row += ySkip) {
            Rectangle rect = new Rectangle(minX, row-trueY+minY, width, 1);
            Raster ras = image.getData(rect);
            if (sourceBands != null) {
                ras = ras.createChild(minX, row-trueY+minY, width, 1, minX, row-trueY+minY,
                                      sourceBands);
            }

            ras.getPixels(minX, row-trueY+minY, width, 1, samples);
    ...
    private void write_IDAT(File[] files) throws IOException {
        IDATOutputStream ios = new IDATOutputStream(stream, 32768);
        try {
            if (metadata.IHDR_interlaceMethod == 1) {
                for (int i = 0; i < 7; i++) {
                    encodePass(ios, files,
                               adam7XOffset[i],
                               adam7YOffset[i],
                               adam7XSubsampling[i],
                               adam7YSubsampling[i]);
    ...
    public void write(File[] files, int[] imageSize) throws IIOException, IOException {
        if (stream == null) {
        ...
        RenderedImage im = ImageIO.read(files[0]);
        SampleModel sampleModel = im.getSampleModel();
        this.numBands = sampleModel.getNumBands();

        this.sourceXOffset = 0;
        this.sourceYOffset = 0;
        this.sourceWidth = imageSize[0];
        this.sourceHeight = imageSize[1];
    ...
    public static void main(String args[]) throws IIOException, IOException {
        if (args.length < 2) {
            System.err.println("PNGStripWriter.class Error: Please provide the size for the output image.");
            System.exit(1);
        }
        
        if (args.length < 3) {
            System.err.println("PNGStripWriter.class Error: Please provide path to at least one input image strip.");
            System.exit(2);
        }
        
        if (args.length < 3) {
            System.err.println("PNGStripWriter.class Error: Please provide output file path.");
            System.exit(2);
        }
        ...
        try {
            imageSize = new int[]{ Integer.parseInt(args[0]), Integer.parseInt(args[1]) };
        } catch (NumberFormatException e) {
            System.err.println("PNGStripWriter.class Error: Invalid output image size: \"(args[0], args[1])\".");
            System.exit(3);
        }
        ...
        ImageOutputStream output = new FileImageOutputStreamSpi().createOutputStreamInstance(outFile);
        
        PNGStripWriter writer = new PNGStripWriter(new PNGStripWriterSpi());
        writer.setOutput(output);
        writer.write(files, imageSize);
    }
}